...
# SSE endpoint only supports raw PCM. We use 22050 Hz s16le.
# Send this in tts_start so the client can create the right AudioContext.
...
class ConversationSession:
    ...
    async def _speak_and_send(self, text: str) -> None:
        ...
        async with self._tts_sem:   # ← now per-session, not global
            ...
            self._speaking = True
            try:
                await self._ws.send_json({"type": "tts_start", "turn_id": turn_id, "encoding": "pcm_s16le", "sample_rate": CARTESIA_SAMPLE_RATE})
                ...
                for attempt in range(1, API_MAX_RETRIES + 1):
                    ...
                    try:
                        ...
                        # Cartesia SDK: tts.sse() is a coroutine - await it
                        # first to get the async iterator, then iterate it.
                        # Cartesia SSE endpoint ONLY supports container="raw"
                        # with a PCM encoding. We use pcm_s16le at 22050 Hz
                        # (good quality, half the bandwidth of 44100).
                        # The client must play raw PCM — see client-side note.
                        ...
                        async for event in sse_stream:
                            ...
                            # SDK yields objects with .audio (bytes) or dicts
                            ...
                            # 8-byte frame: [turn_id uint32][seq uint32] big-endian
                            header = struct.pack(">II", turn_id, seq_in_turn)
                            seq_in_turn += 1
                            await self._ws.send_bytes(header + audio_data)
